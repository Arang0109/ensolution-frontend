...
import { EquipType, PITOT_TUBE_OPTIONS } from '@/entities/agency/equipment/model';
  ...
  const renderSpec = () => {
    switch (equipment.type) {
        ...
        return (
          <div className="grid grid-cols-3 gap-4">
            <div className='text-xs md:text-sm space-y-1'>
              <span className="text-gray-500">적산량</span>
              <p>{spec?.totalVolume ?? '-'} m<sup>3</sup></p>
            </div>
            <div className='text-xs md:text-sm space-y-1'>
              <span className="text-gray-500">오리피스 보정계수</span>
              <p>{spec?.orificeDp ?? '-'}</p>
            </div>
            <div className='text-xs md:text-sm space-y-1'>
      ...
      case EquipType.PITOT_TUBE: {
        ...
        return (
          <div className="space-y-4">
            <div className='text-xs md:text-sm space-y-1'>
              <span className="text-gray-500">피토우관 용도</span>
            ...
            <div className='text-xs md:text-sm space-y-1'>
              <span className="text-gray-500">유속 별 적용되는 피토우관 계수</span>
              <div className="mt-2 grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-2">
                {spec.coefficients.map((coef, idx) => (
                <div key={idx} className="bg-white p-2 rounded border text-xs md:text-sm">
      ...
      case EquipType.NOZZLE: {
        ...
        return (
          <div className='text-xs md:text-sm space-y-1'>
            <span className="text-gray-500">노즐 직경 목록</span>
            {spec?.diameters && spec.diameters.length > 0 ? (
              <div className="mt-2 grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-2">
                {spec.diameters.map((nozzle, idx) => (
                  <span key={idx} className="bg-white px-3 py-1 rounded border text-xs md:text-sm">
                    {nozzle.diameter} cm
                  </span>
                ))}
              </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-start">
        <div>
          <h4 className="font-semibold text-xs md:text-sm text-gray-800 mb-3">상세 스펙</h4>
          {renderSpec()}
        </div>
        <div
          onClick={(e) => {
      ...
      {/* 추가 정보 */}
      <div className="pt-4 border-t border-gray-200 text-xs md:text-sm space-y-1">
