...
import type { MeasurementField, MeasurementType, PlanDetailResponse } from '@/entities/plan/model';
...
export const getDefaultPreInfoEditForm = (
  ...
  return {
    referenceNumber: measurementInfo?.basicInfo.referenceNumber ?? "",
    measureDate: measurementInfo?.basicInfo.measureDate ?? "",
    receivedDate: measurementInfo?.basicInfo.receivedDate ?? "",
    analysisDate: measurementInfo?.basicInfo.analysisDate ?? "",
    measurementField: measurementInfo?.basicInfo.measurementField ?? "AIR",
    measurementType: measurementInfo?.basicInfo.measurementType ?? "SELF",
    teamName: measurementInfo?.team.teamName ?? "",
    vehicleNumber: measurementInfo?.team.vehicleNumber ?? "",
    mentor: measurementInfo?.team.mentor ?? "",
    mentee: measurementInfo?.team.mentee ?? "",

    measureStartTime: measurementInfo?.basicInfo.measureStartTime ?? "",
    measureEndTime: measurementInfo?.basicInfo.measureEndTime ?? "",
