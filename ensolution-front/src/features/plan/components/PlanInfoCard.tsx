import type { PlanDetailResponse, PlanUpdateRequest } from "@plan/model";
...
export const PlanInfoCard = ({
  ...
  const { plan, stack, workplace, company } = planDetail;
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            {isEditMode && editForm && onChange ? (
              ...
              <p className="text-gray-800 font-medium">{plan.teamId}</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            {isEditMode && editForm && onChange ? (
              ...
              <p className="text-gray-800 font-medium">{plan.measurementType}</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-gray-800 font-medium">{STATUS_LABELS[plan.status]}</p>
        </div>

        <div className="pt-4 border-t border-gray-200">
          <p className="text-sm text-gray-500">
            등록일: {new Date(plan.createdAt).toLocaleString('ko-KR')}
          </p>
        </div>
      </div>
    </div>
