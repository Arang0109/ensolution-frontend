...
export const PlanDetailPage = () => {
  ...
  const { planDetail, loading } = usePlanDetail(Number(planId));
  const { isDeleting, handleDelete } = usePlanActions();
  ...
  const handleDeleteClick = async () => {
    ...
    const result = await handleDelete(Number(planId));
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "measurementinfo":
        return <MeasurementInfoTab planDetail={planDetail} />;
      case "preinfo":
        return <PreInfoTab planDetail={planDetail} />;
      case "measurement":
