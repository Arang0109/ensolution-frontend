...
export const MeasurementInfoTab = ({ scheduleDetail }: MeasurementInfoTabProps) => {
  const { schedule, measurements, stack } = scheduleDetail;
  ...
  const handleDeleteMeasurement = async (measurementId: number) => {
    ...
    try {
      const result = await deleteMeasurement(schedule.id, measurementId);
      if (result.status) {
        alert("측정항목이 삭제되었습니다.");
        window.location.reload();
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-2">
              ...
              {isEditing ? (
                ...
                <div className="px-4 py-3 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-sm text-gray-900">{schedule.measurementType}</p>
                </div>
              )}
            </div>
