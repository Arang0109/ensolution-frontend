import { useEquipmentEditForm } from '@equipment/hooks/useEquipmentEditForm';
...
export const EquipmentEditModal = ({ equipment, onClose, onSuccess }: EquipmentEditModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result?.success) {
      ...
      showToast(
        result?.message ?? '장비 수정 중 오류가 발생했습니다.',
        'error'
      );
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">장비 수정</h2>
          <button
            onClick={onClose}
