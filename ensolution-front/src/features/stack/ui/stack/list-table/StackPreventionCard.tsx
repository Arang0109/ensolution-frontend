    ...
    <div className="space-y-4">
      {preventions.map((preventionDetail) => (
        ...
        >
                ...
                {preventionDetail.facilities.map((facility) => (
                  <div key={facility.id} className="bg-slate-50 rounded p-3 text-sm">
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-xs md:text-sm font-bold text-gray-800">{facility.name}</span>
                    </div>
          ...
          {/* 제거대상물질 목록 */}
          <div className="mt-3 pt-3 border-t border-gray-200">
            <h4 className="text-xs md:text-sm font-semibold text-neutral-800 mb-2">
